...
upload_mode = st.radio(
    ...
    index=[" I have embedding CSV", " I have raw think-aloud text CSV"].index(st.session_state.upload_mode)
)
st.session_state.upload_mode = upload_mode

behavior_file = st.file_uploader(" Upload Behavior Data (.csv)", type=["csv"])
if behavior_file:
    st.session_state.behavior_df = pd.read_csv(behavior_file)


if upload_mode == " I have embedding CSV":
    embedding_file = st.file_uploader(" Upload Embedding CSV", type=["csv"])
    if embedding_file:
        st.session_state.embedding_df = pd.read_csv(embedding_file)
    ...
    if text_file and api_key:
        ...
        if st.button(" Generate Embedding from Text"):
            with st.spinner("Generating embeddings using OpenAI..."):
                ...
                st.success("✅ Embedding generated and saved.")


behavior_df = st.session_state.get("behavior_df", None)
embedding_df = st.session_state.get("embedding_df", None)


if behavior_df is not None and embedding_df is not None:
    ...
    if st.button(" Fit Base LSTM & Extract Hidden"):
        with st.spinner("Training base LSTM and extracting hidden states…"):
            ...
            reduced_embeds = np.stack([
                v.numpy() if hasattr(v, "numpy") else v 
                for v in st.session_state.dataset_h0.h0_vecs
            ])
